...
function selectBusStation(select_station_value){
  ...
  if(select_station_value > predict_data[2] && select_station_value <= 8){
    ...
    if(select_station_value == predict_data[2] || (select_station_value == 0 && predict_data[2] == 8)){
      api_data.predict_time = `校巴現時所在位置`
      count_predict_time = 0
    }
  }
  if(parseInt(count_predict_time / 60) > 0 && count_predict_time != 0){
    api_data.predict_time = `約${parseInt(count_predict_time / 60)}分鐘${count_predict_time % 60}秒後到達`
  }else if(count_predict_time != 0){
    api_data.predict_time = `約${count_predict_time}秒後到達`
  }
}
...
function getPrepareStation(latest_data,last_week_data_interlaced){
  ...
  for(let i = i_use ; i < last_week_data_interlaced.length && time_interlaced > 0 ; i++){
    ...
    if(time_interlaced <= 0){
      api_data.current_station = um_stations_new[i][0]
      predict_data[1] = um_stations_new[i]
      predict_data[2] = i
      api_data.next_station = um_stations_new[i][1]
    }else if(time_interlaced > 0 && i+1 == last_week_data_interlaced.length){
      api_data.current_station = um_stations_new[8][0]
      api_data.next_station = um_stations_new[8][1]
      ...
      success: function(data) {
        ...
        if(status == 1){
          if(data._embedded[0]){
            ...
            api_data.latest_bus_data = `校巴最近在${datetime_use[3]}時${datetime_use[4]}分${datetime_use[5]}秒抵達${data._embedded[0].station}巴士站`
          }
...
if(currentTimeline != null){
  ...
  api_data.latest_bus_message = `下一班車會在${currentTimeline.small_time_line.time_to[0]}時${currentTimeline.small_time_line.time_to[1]}分在研究生宿舍巴士站出發`
  predictBusStation(currentTimeline)
}else{
  api_data.current_station = "現在不是校巴服務時間"
  api_data.next_station = "現在不是校巴服務時間"
}
const queryString = window.location.search
const urlParams = new URLSearchParams(queryString)
var stationParameter = urlParams.get('station')

// <option value="0">研究生宿舍PGH</option>
// <option value="1">劉少榮樓E4</option>
// <option value="2">大學會堂N2</option>
// <option value="3">行政樓N6</option>
// <option value="4">科技學院E11</option>
// <option value="5">人民社科樓E21</option>
// <option value="6">法學院E32</option>
// <option value="7">薈萃坊S4</option>
setTimeout((e)=>{
  if(stationParameter){
  selectBusStation(stationParameter)
}
  document.getElementById("api_data").innerText = JSON.stringify(api_data)
},1000)